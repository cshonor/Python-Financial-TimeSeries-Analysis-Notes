# docs · 《Python 时间序列预测》章节笔记

| 章节 | 笔记 | 配套代码 |
|------|------|----------|
| 第 1 章 了解时间序列预测 | [chapter01_time_series_forecasting_intro.md](./chapter01_time_series_forecasting_intro.md) | [chapter01_forecast_workflow_demo.py](../code/chapter01_forecast_workflow_demo.py) |
| 第 2 章 基线预测 | [chapter02_baseline_forecasting.md](./chapter02_baseline_forecasting.md) | [chapter02_baseline_forecast_demo.py](../code/chapter02_baseline_forecast_demo.py) |
| 第 3 章 随机游走 | [chapter03_random_walk.md](./chapter03_random_walk.md) | [chapter03_random_walk_demo.py](../code/chapter03_random_walk_demo.py) |
| 第 4 章 移动平均 MA | [chapter04_moving_average_process.md](./chapter04_moving_average_process.md) | [chapter04_ma_process_demo.py](../code/chapter04_ma_process_demo.py) |
| 第 5 章 自回归 AR | [chapter05_autoregressive_process.md](./chapter05_autoregressive_process.md) | [chapter05_ar_process_demo.py](../code/chapter05_ar_process_demo.py) |
| 第 6 章 ARMA 建模 | [chapter06_arma_modeling.md](./chapter06_arma_modeling.md) | [chapter06_arma_modeling_demo.py](../code/chapter06_arma_modeling_demo.py) |
| 第 7 章 ARIMA | [chapter07_arima_nonstationary.md](./chapter07_arima_nonstationary.md) | [chapter07_arima_demo.py](../code/chapter07_arima_demo.py) |
| 第 8 章 SARIMA 季节性 | [chapter08_sarima_seasonality.md](./chapter08_sarima_seasonality.md) | [chapter08_sarima_demo.py](../code/chapter08_sarima_demo.py) |
| 第 9 章 SARIMAX 外生变量 | [chapter09_sarimax_exogenous.md](./chapter09_sarimax_exogenous.md) | [chapter09_sarimax_demo.py](../code/chapter09_sarimax_demo.py) |
| 第 10 章 VAR 多变量 | [chapter10_var_multivariate.md](./chapter10_var_multivariate.md) | [chapter10_var_demo.py](../code/chapter10_var_demo.py) |
| 第 11 章 顶点项目 | [chapter11_capstone_aus_drugs.md](./chapter11_capstone_aus_drugs.md) | [chapter11_capstone_demo.py](../code/chapter11_capstone_demo.py) |
| 第 12 章 深度学习入门 | [chapter12_deep_learning_intro.md](./chapter12_deep_learning_intro.md) | [chapter12_dl_preprocess_demo.py](../code/chapter12_dl_preprocess_demo.py) |
| 第 13 章 数据窗口与基线 | [chapter13_data_window_baselines.md](./chapter13_data_window_baselines.md) | [chapter13_data_window_baseline_demo.py](../code/chapter13_data_window_baseline_demo.py) |
| 第 14 章 线性模型与 DNN | [chapter14_linear_and_dnn.md](./chapter14_linear_and_dnn.md) | [chapter14_linear_dnn_demo.py](../code/chapter14_linear_dnn_demo.py) |
| 第 15 章 LSTM | [chapter15_lstm.md](./chapter15_lstm.md) | [chapter15_lstm_demo.py](../code/chapter15_lstm_demo.py) |
| 第 16 章 CNN | [chapter16_cnn.md](./chapter16_cnn.md) | [chapter16_cnn_demo.py](../code/chapter16_cnn_demo.py) |
| 第 17 章 ARLSTM | [chapter17_arlstm.md](./chapter17_arlstm.md) | [chapter17_arlstm_demo.py](../code/chapter17_arlstm_demo.py) |
| 第 18 章 DL 顶点项目 | [chapter18_capstone_household_power.md](./chapter18_capstone_household_power.md) | [chapter18_capstone_power_demo.py](../code/chapter18_capstone_power_demo.py) |
| 第 19 章 Prophet | [chapter19_prophet.md](./chapter19_prophet.md) | [chapter19_prophet_demo.py](../code/chapter19_prophet_demo.py) |
| 第 20 章 收官顶点 | [chapter20_capstone_steak_price.md](./chapter20_capstone_steak_price.md) | [chapter20_capstone_steak_demo.py](../code/chapter20_capstone_steak_demo.py) |
| 第 21 章 全书总结 | [chapter21_beyond_self_summary.md](./chapter21_beyond_self_summary.md) | [chapter21_summary_guide_demo.py](../code/chapter21_summary_guide_demo.py) |

**笔记结构**：章节总览 → 书本提炼 → 生命周期速查 → 演示 → 量化衔接 → 自检 → 留白。

**数据处理前置**：[`Python-Data-Analysis/docs/chapter11_time_series.md`](../../Python-Data-Analysis/docs/chapter11_time_series.md)
